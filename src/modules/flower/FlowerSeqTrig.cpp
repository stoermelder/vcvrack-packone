...
struct FlowerTrigModule : FlowerChainModule {
	...
	FlowerTrigModule() {
		...
		auto pq1 = configParam<TrigStepModeParamQuantity<SEQ>>(PARAM_STEPMODE, 0.f, 1.f, 0.f, "Mode");
		pq1->engine = &seq;

		auto pq2 = configParam<TrigFlowerKnobParamQuantity<SEQ>>(PARAM_STEP_CENTER, -std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity(), 0.f);
		pq2->engine = &seq;

		for (int i = 0; i < STEPS; i++) {
			configParam(PARAM_STEP + i, 0.f, 1.f, 0.1f, string::f("Step %i length", i + 1), "", 0.f);

			auto pq2 = configParam<TrigStepButtonParamQuantity<SEQ>>(PARAM_STEP_BUTTON + i, 0.f, 1.f, 0.f);
			pq2->engine = &seq;
			pq2->i = i;
		}
...
struct FlowerTrigWidget : ThemedModuleWidget<FlowerTrigModule<16, 8, 8>> {
	...
	FlowerTrigWidget(MODULE* module)
		: ThemedModuleWidget<MODULE>(module, "FlowerSeqTrig") {
		setModule(module);
...
Model* modelFlowerSeqTrig = createModel<StoermelderPackOne::Flower::FlowerTrigModule<16, 8, 8>, StoermelderPackOne::Flower::FlowerTrigWidget>("FlowerSeqTrig");