...
struct ReMoveWidget : ThemedModuleWidget<ReMoveModule> {
	...
	void appendContextMenu(Menu* menu) override {
		...
		menu->addChild(StoermelderPackOne::Rack::createMapSubmenuItem<int>("# of sequences",
			{
				{ 1, "1" },
				{ 2, "2" },
				{ 4, "4" },
			...
			[=]() { return module->seqCount; },
			[=](int seqCount) { if (!module->isRecording) module->seqResize(seqCount); }
		));

		menu->addChild(StoermelderPackOne::Rack::createMapPtrSubmenuItem<SEQCHANGEMODE>("Sequence change mode",
			{
				{ SEQCHANGEMODE_RESTART, "Restart" },
				{ SEQCHANGEMODE_OFFSET, "Offset" }
			},
			&module->seqChangeMode
		));

		menu->addChild(StoermelderPackOne::Rack::createMapSubmenuItem<RECMODE>("Record mode",
			{
				{ RECMODE_TOUCH, "Touch" },
				{ RECMODE_MOVE, "Move" },
				{ RECMODE_MANUAL, "Manual" },
				{ RECMODE_SAMPLEHOLD, "Sample & Hold" }
			},
			[=]() { return module->recMode; },
			[=](RECMODE recMode) { if (!module->isRecording) module->recMode = recMode; }
		));

		menu->addChild(createBoolPtrMenuItem("Autoplay after record", "", &module->recAutoplay));

		menu->addChild(StoermelderPackOne::Rack::createMapPtrSubmenuItem<PLAYMODE>("Play mode",
			{
				{ PLAYMODE_LOOP, "Loop" },
				{ PLAYMODE_ONESHOT, "Oneshot" },
				{ PLAYMODE_PINGPONG, "Ping Pong" },
				{ PLAYMODE_SEQLOOP, "Sequence loop" },
				{ PLAYMODE_SEQRANDOM, "Sequence random" }
			},
			&module->playMode
		));

		menu->addChild(new MenuSeparator());
		menu->addChild(StoermelderPackOne::Rack::createMapSubmenuItem<SEQCVMODE>("Port SEQ# mode",
			{
				{ SEQCVMODE_10V, "0..10V" },
				{ SEQCVMODE_C4, "C4-G4" },
				{ SEQCVMODE_TRIG, "Trigger" }
			},
			[=]() { return module->seqCvMode; },
			[=](SEQCVMODE seqCvMode) { if (!module->isRecording) module->seqCvMode = seqCvMode; }
		));

		menu->addChild(StoermelderPackOne::Rack::createMapPtrSubmenuItem<RUNCVMODE>("Port RUN mode",
			{
				{ RUNCVMODE_GATE, "Gate" },
				{ RUNCVMODE_TRIG, "Trigger" }
			},
			&module->runCvMode
		));

		menu->addChild(StoermelderPackOne::Rack::createMapSubmenuItem<RECOUTCVMODE>("Port REC-out mode",
			{
				{ RECOUTCVMODE_GATE, "Gate" },
				{ RECOUTCVMODE_TRIG, "Trigger" }
			},
			[=]() { return module->recOutCvMode; },
			[=](RECOUTCVMODE recOutCvMode) { if (!module->isRecording) module->recOutCvMode = recOutCvMode; }
		));

		menu->addChild(StoermelderPackOne::Rack::createMapSubmenuItem<INCVMODE>("Port IN voltage",
			{
				{ INCVMODE_UNI, "0V..10V" },
				{ INCVMODE_BI, "-5V..5V" }
			},
			[=]() { return module->inCvMode; },
			[=](INCVMODE inCvMode) { if (!module->isRecording) module->inCvMode = inCvMode; }
		));

		menu->addChild(StoermelderPackOne::Rack::createMapSubmenuItem<OUTCVMODE>("Port OUT voltage",
			{
				{ OUTCVMODE_CV_UNI, "CV with 0V..10V" },
				{ OUTCVMODE_CV_BI, "CV with -5V..5V" },
				{ OUTCVMODE_EOC, "EOC" }
			},
			[=]() { return module->outCvMode; },
			[=](OUTCVMODE outCvMode) { if (!module->isRecording) module->outCvMode = outCvMode; }
		));
	}
};
