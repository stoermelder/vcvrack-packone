// PILEPOLY test cases. Included by PilePoly.test.cpp inside namespace __module.
// Not a standalone header: PilePoly.test.hpp supplies everything these cases use.

TEST_CASE("Construction and initialization", "[PilePoly]") {
	Test::Harness h;
	PilePolyModule* m = h.addModule<PilePolyModule>("PilePoly");
...
TEST_CASE("Preset JSON null-guards", "[PilePoly][JSON]") {
	Test::Harness h;
	auto module = h.addModule<PilePolyModule>("PilePoly");
...
TEST_CASE("JSON round-trip preserves state", "[JSON][PilePoly]") {
	...
	auto moduleNew = h.addModule<PilePolyModule>("PilePoly");
