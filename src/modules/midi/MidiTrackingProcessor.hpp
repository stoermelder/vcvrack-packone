...
struct MidiTrackingProcessor : MidiProcessorHandler {
	...
	MidiTrackingProcessorHandler* handler;
	...
	bool processMidi(const MessageEx& msg) {
		switch (msg.type) {
	...
	void processNoteOn(const MessageEx& msg) {
		...
		if (learnActive && vel > 0) {
			...
			disableMapLearn();
			handler->processMapLearn(MidiTrackingType::NOTE, learnId);
			return;
		}
		// Send
		for (uint16_t id : mapNote[note]) {
			handler->processMapUpdate(MidiTrackingType::NOTE, id, vel);
		}
	}
	...
	void processNoteOff(const MessageEx& msg) {
		uint8_t note = msg.getNote();
		for (uint16_t id : mapNote[note]) {
			handler->processMapUpdate(MidiTrackingType::NOTE, id, 0);
		}
	}
	...
	void processCc(const MessageEx& msg) {
		...
		if (learnActive && value > 0) {
			...
			disableMapLearn();
			handler->processMapLearn(MidiTrackingType::CC, learnId);
			return;
		}
		// Send
		for (uint16_t id : mapCc[cc]) {
			handler->processMapUpdate(MidiTrackingType::CC, id, value);
		}
	}
