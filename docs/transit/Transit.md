...
A set's snapshot bindings can be copied to another set: right-click the source set-button and choose _Copy_, then right-click the target set-button and choose _Paste_. If _Snapshot-set node positions_ (see [Context menu](#context-menu)) is set to _Store_ or _Auto_, the pad-point layout and _Mix_ point position are copied along with the bindings; if _Snapshot-set motion sequence_ is enabled, the selected motion-sequence is copied along as well. The set's color and label are never affected by _Copy_/_Paste_.
...
By default the selected sequence (_Slot_) is also shared by all snapshot-sets, same as the pad-point layout. The _Snapshot-set motion sequence_ context-menu option (see [Context menu](#context-menu)) can be enabled to have each snapshot-set remember its own selected sequence instead: switching sets then also switches which of the 16 sequences is active.
...
- **Snapshot-set motion sequence** — toggle whether each snapshot-set remembers its own selected motion-sequence (see [Motion-Sequences](#motion-sequences)), on top of the snapshot bindings and (if enabled) pad-point layout it always/optionally stores. Off by default, matching the shared-sequence behaviour described in [Snapshot-sets](#snapshot-sets). Enabling it seeds every set with whichever sequence is currently selected, so switching sets right afterwards doesn't jump to sequence 1 for sets that haven't been touched yet.
