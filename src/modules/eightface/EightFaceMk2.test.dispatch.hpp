// Stability & performance mode dispatch, auto mode, and threading regression tests. The mode must
// select a single hop --
...
// result-only assertion passes under a chained two-hop bug too.
...
TEST_CASE("needsGuiThread override takes the UI hop directly, without going through the worker", "[EightFaceMk2][dispatch]") {
	...
	// Chaining worker -> guiTasks would also land it on the UI thread, but it re-creates the
	// two-hop shape the single-hop dispatch design exists to remove, and it enqueues onto
	// guiTasks from the worker thread, violating GuiTaskProcessor's single-producer contract
	// (GuiTaskProcessor.hpp: "Only the engine thread may call enqueue()").
...
TEST_CASE("GUI-thread entry points defer through a request instead of enqueueing directly", "[EightFaceMk2][dispatch]") {
	// GuiTaskProcessor is single-producer, but faceSlotCmd(SLOT_CMD::LOAD, ...) is called from
	// the GUI thread (Shift+click, slot menu "Load"). It calls presetLoadRequest(), which only
	// records the request; process() performs presetLoad()/dispatch() on the engine thread.
	DispatchFixture f(createEightFaceMk2Module);
	f.m->dispatch.guiSafeMode = GUISAFEMODE::GUI_WITH_LOCK;
	f.savePreset("#050505");
...
TEST_CASE("GUI vs GUI_WITH_LOCK apply through different objects", "[EightFaceMk2][dispatch]") {
	...
	// (GUI_WITH_LOCK / Safe) -- observable without threading. For this module both objects end up
	// updating the same module state (ModuleWidget::fromJson itself just calls
	// APP->engine->moduleFromJson(module, ...), which calls module->fromJson()), so the two
	// branches are asserted by exercising each one distinctly rather than by a differing effect --
	// see the comment on each SECTION.
...
TEST_CASE("No window: presets still load in every mode", "[EightFaceMk2][dispatch]") {
	// The branch that used to read settings::isPlugin && !APP->window directly has been deleted --
	// GuiTaskProcessor now handles "nobody will call step()" itself, starting a private worker the
	// instant vcv::ui::hasWindow() is false. This is the branch most likely to regress: presets
	// must still load with the editor closed.
...
TEST_CASE("No window: an allowlisted module is skipped in every mode", "[EightFaceMk2][dispatch]") {
	...
	// loading is the deliberate trade -- it contradicts the general "presets must still load in
	// every mode" rule for this specific configuration, and is preferred over applying the module
	// with no UI thread to apply it on.
	auto worker = std::make_shared<CountingSyncTaskWorker>();
	DispatchFixture f([&]() { return createEightFaceMk2ModuleWithWorker(worker); });
	f.h.setUiMode(Test::UiMode::UiAbsent);
	f.m->boundModules[0]->needsGuiThread = true;
...
TEST_CASE("Two presetLoad() calls between UI frames both apply, in order", "[EightFaceMk2][dispatch]") {
	// mk1's old single-slot workerGuiModuleWidget handoff silently dropped the first of two loads
	// issued in the same UI frame. mk2's dispatch() enqueues into GuiTaskProcessor<32> and
	// captures `p` by value per task (PresetDispatch.hpp:79-95), so this must not happen here --
	// but it had no direct test pinning it. Two engine-thread presetLoad() calls, no drain in
	// between, must both still land, in the order they were issued (the second's effect must not
	// depend on the first having applied).
	DispatchFixture f(createEightFaceMk2Module);
	f.m->dispatch.guiSafeMode = GUISAFEMODE::GUI_WITH_LOCK;
	...
	EightFaceMk2Slot* slot1 = f.m->faceSlot(1);
	NVGcolor prevColor = f.boundM->boxColor;
	f.boundM->boxColor = color::fromHexString("#a20000");
	json_t* vJ = f.boundM->toJson();
	f.boundM->boxColor = prevColor;
	slot1->preset->push_back(vJ);
	*(slot1->presetSlotUsed) = true;
	...
	// the two-tasks-queued case a single-slot handoff could not represent at all.
	f.m->presetLoad(0, false, true);
	f.m->presetLoad(1, false, true);
	...
	f.h.uiFrame();
	REQUIRE(f.appliedLabel() == "#a20000");
	REQUIRE(f.m->dispatch.pendingGuiTasks() == 0);
}

TEST_CASE("Closing then reopening the editor retires and restarts the dispatch worker", "[EightFaceMk2][dispatch]") {
	// GuiTaskProcessor starts a private worker the instant vcv::ui::hasWindow() is false (its own
	// process(), pumped from EightFaceMk2's process() via guiTaskDivider) and retires it once a
	// window reappears (GuiTaskProcessor.hpp's retireWorker(), Running -> Retiring). Toggling
	// UiMode mid-test exercises the full Running -> Retiring -> Running cycle a user gets from
	// closing and reopening the plugin editor, which the other [dispatch] cases (each fixed at one
	// UiMode for their whole body) never do.
	DispatchFixture f(createEightFaceMk2Module);
	f.m->dispatch.guiSafeMode = GUISAFEMODE::GUI_WITH_LOCK;
	...
	EightFaceMk2Slot* slot1 = f.m->faceSlot(1);
	NVGcolor prevColor = f.boundM->boxColor;
	f.boundM->boxColor = color::fromHexString("#b20000");
	json_t* vJ = f.boundM->toJson();
	f.boundM->boxColor = prevColor;
	slot1->preset->push_back(vJ);
	*(slot1->presetSlotUsed) = true;
	...
	f.m->presetLoad(1, false, true);
	f.h.uiFrame();
	REQUIRE(f.appliedLabel() == "#b20000");

	// One full close/reopen/close cycle: the worker must be able to start again, proving retiring
	// left it in a state startWorker() knows how to reclaim (GuiTaskProcessor.hpp's Absent/Retiring
	// -> Starting compare_exchange), not stuck.
	f.h.setUiMode(Test::UiMode::UiAbsent);
	EightFaceMk2Slot* slot2 = f.m->faceSlot(2);
	f.boundM->boxColor = color::fromHexString("#b30000");
	json_t* vJ2 = f.boundM->toJson();
	f.boundM->boxColor = prevColor;
	slot2->preset->push_back(vJ2);
	*(slot2->presetSlotUsed) = true;

	f.m->presetLoad(2, false, true);
	for (int i = 0; i < 10 && f.appliedLabel() != "#b30000"; i++) {
		f.h.dspSteps(256);
	}
	REQUIRE(f.appliedLabel() == "#b30000");
	REQUIRE(f.m->dispatch.guiTasks.workerState.load() == WorkerState::Running);
...
TEST_CASE("A load enqueued while guiTasks is full is dropped, not applied, and does not corrupt state", "[EightFaceMk2][dispatch]") {
	// GuiTaskProcessor<32>::enqueue() drops silently when the ring buffer is full (TaskProcessor
	// .hpp:19: "if (!queue.full()) ... return false"), and PresetDispatch::dispatch() does not
	// check that return value (PresetDispatch.hpp:79-95: "guiTasks.enqueue([...]) {...}" with the
	// bool discarded). The existing "Queue capacity" test proves 32 is enough headroom for one
	// preset's worth of bound modules; this proves the OTHER side of TaskProcessor.hpp:19's
	// contract -- once actually full, a further enqueue is silently dropped rather than blocking,
	// overwriting a queued task, or corrupting the ring buffer -- by filling the queue directly and
	// confirming the drop leaves everything already queued intact.
	DispatchFixture f(createEightFaceMk2Module);
	f.m->dispatch.guiSafeMode = GUISAFEMODE::GUI_WITH_LOCK;
...
TEST_CASE("A legacy guiSafeMode == 0 (WORKER) patch still loads and dispatches through the worker", "[EightFaceMk2][JSON][dispatch]") {
	// guiSafeMode moved from a plain module field to dispatch.guiSafeMode (PresetDispatch.hpp:64)
	// during the mk2 dispatch refactor, but the stored integer values and their meaning did not
	// change: GUISAFEMODE::WORKER is 0 both before and after (PresetDispatch.hpp:11-15's
	// declaration order matches the pre-refactor enum, EightFaceMk2.cpp git history confirms it was
	// never renumbered). A patch saved before the refactor must still deserialize to the same mode
	// and actually dispatch through the worker, not just decode to the right enum value -- and since
	// WORKER is also dataFromJson()'s fallback when the key is absent entirely (:828), that absent-
	// key legacy case is covered here too, not just an explicit stored 0.
	Test::ModuleScaffold<EightFaceMk2Module<8>> mods{createEightFaceMk2Module};
	EightFaceMk2Module<8>* m = mods.create("EightFaceMk2");
	...
	auto worker = std::make_shared<CountingSyncTaskWorker>();
	EightFaceMk2Module<8>* m2 = mods.adopt(new EightFaceMk2Module<8>(worker));
	m2->model = modelEightFaceMk2;
	...
	EightFaceMk2Module<8>* boundM = mods.create("EightFaceMk2");
	EightFaceMk2Widget<8>* boundMw = Test::createWidget<EightFaceMk2Widget<8>>(boundM);
	bindForTest(m2, boundM, boundMw);

	EightFaceMk2Slot* slot = m2->faceSlot(0);
	slot->preset->push_back(boundM->toJson());
	*(slot->presetSlotUsed) = true;
	m2->presetPrev = -1;

	m2->process(Test::makeProcessArgs(0));
	m2->presetLoad(0, false, true);
	REQUIRE(worker->count == 1);
