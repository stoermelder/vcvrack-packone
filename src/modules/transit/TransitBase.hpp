...
struct TransitCtrlMaster {
	virtual ~TransitCtrlMaster() = default;
 ...
 *  TransitCtrlMaster pointer into TransitCtrl via this interface when it finds TransitCtrl
 *  as the immediate right neighbor. */
struct TransitCtrlReceiver {
	virtual ~TransitCtrlReceiver() = default;
	virtual void setTransitCtrl(TransitCtrlMaster* ctrl) = 0;
