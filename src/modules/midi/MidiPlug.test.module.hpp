// MIDIPLUG test cases. Included by MidiPlug.test.cpp inside namespace __module.
// Not a standalone header: MidiPlug.test.hpp supplies everything these cases use.

TEST_CASE("Construction and reset", "[MidiPlug]") {
	Test::Harness h;
	auto module = h.addModule<Module2>("MidiPlug");
...
TEST_CASE("JSON round-trip preserves state", "[MidiPlug][JSON]") {
	Test::Harness h;
	Module2* m = h.addModule<Module2>("MidiPlug");
	Module2* m2 = h.addModule<Module2>("MidiPlug");
...
TEST_CASE("Routes both inputs to both outputs", "[MidiPlug]") {
	...
	feed(h, module, 0, Test::makeMidiMessage(0xb, 2, 7, 64));
	feed(h, module, 1, Test::makeMidiMessage(0x9, 4, 60, 100));
...
TEST_CASE("Thru passes messages unchanged", "[MidiPlug]") {
	...
	feed(h, module, 0, Test::makeMidiMessage(0xb, 9, 1, 127));
...
TEST_CASE("REPLACE rewrites channel", "[MidiPlug]") {
	...
	SECTION("Channel-voice messages are rewritten to the target channel") {
		...
		for (uint8_t st : statuses) {
			feed(h, module, 0, Test::makeMidiMessage(st, 2, 40, 80));
...
TEST_CASE("FILTER keeps only the target channel", "[MidiPlug]") {
	...
	feed(h, module, 0, Test::makeMidiMessage(0xb, 3, 7, 64));
	// Non-matching channel is dropped.
	feed(h, module, 0, Test::makeMidiMessage(0xb, 4, 7, 64));
...
TEST_CASE("processBypass drains both input queues without sending output", "[MidiPlug]") {
	...
	// Queues are drained; a subsequent normal dsp step has nothing left to forward.
	h.dspStep();
	REQUIRE(out0->sent.empty());
	REQUIRE(out1->sent.empty());
