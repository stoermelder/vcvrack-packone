...
// Lazy-loaded on first access to avoid allocating ~400KB of string literals unnecessarily.
inline const std::vector<AutoTagRule>& getAutoTagRules() {
...
AutoTagResult customTagAuto(const std::vector<AutoTagRule>& rules = getAutoTagRules(), const std::vector<Plugin*>& plugins = rack::plugin::plugins);
