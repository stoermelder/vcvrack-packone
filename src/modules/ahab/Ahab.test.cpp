...
TEST_CASE("Integration test - successive E bang separation #426", "[Ahab]") {
	AhabModule* m = Test::createModule<AhabModule>("Ahab");
	Test::registerModule(m);

	json_error_t jerr;
	json_t* presetJ = json_loads(Ahab_vcvm_test1, 0, &jerr);
	REQUIRE(presetJ != nullptr);
	...
	// Ensure a sane tempo for deterministic tick advancement.
	m->params[AhabModule::BPM_PARAM].setValue(60.0f);
	int framesPerTick = (int)APP->engine->getSampleRate();
	int cycles = 0;
	Glyph const* field = m->sim->getFieldBuffer();

	m->sim->setGlyphRequest(0, 0, 'E'); // initial bang to trigger processing
	for (int i = 0; i < framesPerTick / 2; ++i) {
		m->process(Test::makeProcessArgs(cycles));
		cycles++;
	}
	REQUIRE(field[2] == 'E'); // Verify initial glyph is set
	...
	REQUIRE(field[6] == 'E'); // Verify glyph has moved according to pattern
	REQUIRE(field[8] == 'E');

	for (int i = 0; i < framesPerTick / 4; ++i) {
		m->process(Test::makeProcessArgs(cycles));
		cycles++;
	}
	REQUIRE(field[8] == '*'); // Verify bang appears at expected location
