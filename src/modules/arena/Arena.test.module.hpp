// Arena.test.module.hpp — construction, JSON round-trip and the pre-existing
// DSP/widget-adjacent coverage (proximity mixing, distance matrix, node/cursor
// setters). Everything that predates the topic-file split; new coverage
// belongs in its own Arena.test.<topic>.hpp file alongside this one.
...
TEST_CASE("Preset JSON null-guards", "[Arena][JSON]") {
	Test::Harness h;
	auto module = h.addModule<ArenaModule<8, 4>>("Arena");
...
TEST_CASE("JSON round-trip preserves module state", "[Arena]") {
	...
	auto* m2 = h.addModule<MODULE>("Arena");
	m2->dataFromJson(j);
	json_decref(j);
	...
	SECTION("First process() after dataFromJson does not overwrite restored MIX params") {
		...
		auto* m3 = h.addModule<MODULE>("Arena");
		for (int i = 0; i < MIX_PORTS; i++) {
