...
struct MidiCatParam : ScaledMapParam<int> {
	...
	/** [Stored to Json] User-defined values for CCMODE::TOGGLE_STEPS / NOTEMODE::TOGGLE_STEPS,
	 *  expressed in limit space (the same space as getLimitMin()/getLimitMax()). Each press
	 *  advances to the next value, wrapping back to the first after the last. */
	std::vector<int> stepValues;
	/** Index into stepValues last emitted. Not stored to Json: replaying a preset should
	 *  start from the first step, not wherever the mapping happened to be left. */
	int stepIndex = 0;
	...
	void reset(bool resetSettings = true) override {
		if (resetSettings) {
			...
			clockSource = 0;
			stepValues = { getLimitMin(), getLimitMax() };
		}
		lightFirstId = -1;
		lightNumColors = 0;
		stepIndex = 0;
	...
	/** Advance to the next user-defined step value and return it in limit space, or -1 if no
	 *  steps are defined. Called once per rising edge / note-on, mirroring the TOGGLE modes. */
	int nextStepValue() {
		if (stepValues.empty()) return -1;
