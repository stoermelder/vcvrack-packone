// RAW test cases. Included by Raw.test.cpp inside namespace __module.
// Not a standalone header: Raw.test.hpp supplies everything these cases use.

TEST_CASE("Construction and initialization", "[Raw]") {
	Test::ModuleScaffold<RawModule> mods;
	RawModule* module = mods.create("Raw");
	RawWidget* mw = Test::createWidget<RawWidget>(module);

	Test::destroyWidget(mw);
}

TEST_CASE("Preset JSON null-guards", "[Raw][JSON]") {
	Test::ModuleScaffold<RawModule> mods;
...
TEST_CASE("Preset JSON clamps out-of-range scalars", "[Raw][JSON]") {
	Test::Harness h;
