...
static json_t* buildPluginJson(const Settings& s) {
	...
	json_object_set_new(mbJ, "arrowKeyNavigation", json_boolean(s.mbArrowKeyNavigation));
...
static void parsePluginJson(json_t* j, Settings& s) {
	...
	if (mbJ) {
		...
		v = json_object_get(mbJ, "arrowKeyNavigation");  if (v) s.mbArrowKeyNavigation = json_boolean_value(v);
