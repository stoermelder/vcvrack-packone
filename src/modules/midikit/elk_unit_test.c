...
static void test_arrays(void) {
  struct js *js;
  char mem[sizeof(*js) + 8000];
  assert((js = js_create(mem, sizeof(mem))) != NULL);

  assert(ev(js, "[]", "{}"));
  assert(ev(js, "let a = []", "undefined"));
  assert(ev(js, "a.length", "0"));

  /* typeof for arrays */
  assert(ev(js, "typeof []", "\"object\""));
  assert(ev(js, "typeof [1]", "\"object\""));
  assert(ev(js, "let aa = [1,2,3]; typeof aa", "\"object\""));

  /* auto-updating length behaviour */
  assert(ev(js, "let h = []", "undefined"));
  assert(ev(js, "h[10] = 5", "5"));
  assert(ev(js, "h.length", "11"));
  assert(ev(js, "let n = []", "undefined"));
  assert(ev(js, "n[-1] = 2", "2"));
  assert(ev(js, "n.length", "0"));

  assert(ev(js, "[1]", "{\"length\":1,\"0\":1}"));
  assert(ev(js, "[1,2,3]", "{\"length\":3,\"2\":3,\"1\":2,\"0\":1}"));
  assert(ev(js, "let b = [1,2,3]", "undefined"));
  assert(ev(js, "b.length", "3"));

  assert(ev(js, "b[0]", "1"));
  assert(ev(js, "b[1]", "2"));
  assert(ev(js, "b[2]", "3"));

  assert(ev(js, "b[0] = 10", "10"));
  assert(ev(js, "b[0]", "10"));
  assert(ev(js, "b[3] = 4", "4"));
  assert(ev(js, "b[3]", "4"));
  assert(ev(js, "b.length", "4"));

  assert(ev(js, "let e = []", "undefined"));
  assert(ev(js, "e[2] = 5", "5"));
  assert(ev(js, "e[2]", "5"));
  assert(ev(js, "e.length", "3"));

  assert(ev(js, "let f = []", "undefined"));
  assert(ev(js, "let ii = 2", "undefined"));
  assert(ev(js, "f[ii] = 7", "7"));
  assert(ev(js, "f.length", "3"));

  assert(ev(js, "let g = [[1,2]]", "undefined"));
  assert(ev(js, "g[0][2] = 3", "3"));
  assert(ev(js, "g[0][2]", "3"));
  assert(ev(js, "g[0].length", "3"));

  assert(ev(js, "let o = {}", "undefined"));
  assert(ev(js, "o['1'] = 2", "2"));
  assert(ev(js, "o['1']", "2"));
  assert(ev(js, "o.length", "undefined"));

  assert(ev(js, "let c = [1, 'hello', true]", "undefined"));
  assert(ev(js, "c[0]", "1"));
  assert(ev(js, "c[1]", "\"hello\""));
  assert(ev(js, "c[2]", "true"));
  assert(ev(js, "c.length", "3"));

  assert(ev(js, "let d = [[1,2], [3,4]]", "undefined"));
  assert(ev(js, "d[0][0]", "1"));
  assert(ev(js, "d[0][1]", "2"));
  assert(ev(js, "d[1][0]", "3"));
  assert(ev(js, "d[1][1]", "4"));

  assert(ev(js, "let i = 1", "undefined"));
  assert(ev(js, "b[i]", "2"));
  assert(ev(js, "b[i+1]", "3"));

  /* Object storing tests */
  assert(ev(js, "let arr = [{}]", "undefined"));
  assert(ev(js, "arr.length", "1"));
  assert(ev(js, "arr[0].a = 5", "5"));
  assert(ev(js, "arr[0].a", "5"));
  assert(ev(js, "arr[1] = {b:2}", "{\"b\":2}"));
  assert(ev(js, "arr[1].b", "2"));
  assert(ev(js, "arr.length", "2"));

  assert(ev(js, "let arr2 = [{a:1}, {}]", "undefined"));
  assert(ev(js, "arr2.length", "2"));
  assert(ev(js, "arr2[0].a", "1"));

  // Test setting length on arrays should error
  assert(ev(js, "let arr3 = [1,2,3]; arr3.length = 1", "ERROR: cannot set array length"));
  // Test setting length on regular objects should work
  assert(ev(js, "let obj = {}; obj.length = 5; obj.length", "5"));
  // Test that length is still readable on arrays
  assert(ev(js, "arr3.length", "3"));

  // Accessing non-existent indices returns undefined
  assert(ev(js, "let empty = []", "undefined"));
  assert(ev(js, "empty[0]", "undefined"));
  assert(ev(js, "empty[100]", "undefined"));

  // Sparse arrays
  assert(ev(js, "let sparse = []; sparse[5] = 42; sparse.length", "6"));
  assert(ev(js, "sparse[0]", "undefined"));
  assert(ev(js, "sparse[5]", "42"));

  // Large indices
  assert(ev(js, "let large = []; large[1000] = 'big'; large.length", "1001"));
  assert(ev(js, "large[1000]", "\"big\""));

  // Array literals with expressions
  assert(ev(js, "let x = 1; [x, x+1, x*3]", "{\"length\":3,\"2\":3,\"1\":2,\"0\":1}"));

  // Indexing with expressions
  assert(ev(js, "let idx = 1; b[idx]", "2"));
  assert(ev(js, "b[idx+1]", "3"));

  // String indices (treated as properties, not affecting length)
  assert(ev(js, "let stridx = []; stridx['0'] = 'zero'; stridx.length", "0"));
  assert(ev(js, "stridx['0']", "\"zero\""));
  assert(ev(js, "stridx[0]", "\"zero\""));

  // Negative indices (treated as properties)
  assert(ev(js, "let neg = []; neg[-1] = 'minus'; neg.length", "0"));
  assert(ev(js, "neg[-1]", "\"minus\""));

  // Array as object property
  assert(ev(js, "let container = {arr: [1,2]}; container.arr.length", "2"));
  assert(ev(js, "container.arr[1]", "2"));

  // Modifying array elements with expressions
  assert(ev(js, "let mod = [10,20]; mod[0] += 5; mod[0]", "15"));
  assert(ev(js, "mod.length", "2"));

  // Empty array literal
  assert(ev(js, "let empty2 = []; empty2.length", "0"));

  // Single element array
  assert(ev(js, "let single = [42]; single.length", "1"));
  assert(ev(js, "single[0]", "42"));
}
