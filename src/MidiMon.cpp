			...
			case 0xf: // system
				if (!midiLogMessages.full()) {
					switch (msg.getChannel()) {
						case 0x8: // clock
							if (showClockMsg) midiLogMessages.push(std::make_tuple(timestamp, "clock tick"));
							break;
						case 0xa: // start
							if (showSystemMsg) midiLogMessages.push(std::make_tuple(timestamp, "start"));
							break;
						case 0xb: // continue
							if (showSystemMsg) midiLogMessages.push(std::make_tuple(timestamp, "continue"));
							break;
						case 0xc: // stop
							if (showSystemMsg) midiLogMessages.push(std::make_tuple(timestamp, "stop"));
							break;
						default:
							break;
					}
