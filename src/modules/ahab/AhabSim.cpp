...
void AhabSim::fromJson(json_t* rootJ) {
	...
	mbuf_reusable_ensure_size(&mbuf_, tmp.height, tmp.width);
	field_deinit(&tmp);
...
bool AhabSim::pushUndo() {
	...
	UndoNode node;
	field_init(&node.f);
	field_copy(&field_, &node.f);
	node.tick = tick_number_.load();
	undo_history_.push_back(std::move(node));
	return true;
}

void AhabSim::getDisplayBuffer(Usz& height, Usz& width) const {
	// Lock-free read from widget thread
	height = field_.height;
	width = field_.width;
}

Glyph const* AhabSim::getFieldBuffer() const {
	return field_.buffer;
}

Mark const* AhabSim::getMbufBuffer() const {
	return mbuf_.buffer;
}
...
// DSP thread operation - cut (clear) a rectangular region in the field buffer
bool AhabSim::cutRect(Usz y, Usz x, Usz h, Usz w) {
	// Clear the specified region
	if (y < field_.height && x < field_.width) {
		Usz clip_h = h;
		Usz clip_w = w;
		if (y + clip_h > field_.height) clip_h = field_.height - y;
		if (x + clip_w > field_.width) clip_w = field_.width - x;
		if (clip_h > 0 && clip_w > 0) {
			gbuffer_fill_subrect(field_.buffer, field_.height, field_.width, y, x, clip_h, clip_w, '.');
		}
	}
	return true;
}
...
// DSP thread operation - fill a rectangular region in the field buffer
bool AhabSim::fillRect(Usz y, Usz x, Usz h, Usz w, Glyph fill) {
	// Check dimensions from the field buffer
	if (y >= field_.height || x >= field_.width) {
		return false;
	}
	if (y + h > field_.height) h = field_.height - y;
	if (x + w > field_.width) w = field_.width - x;
	// Fill the buffer
	gbuffer_fill_subrect(field_.buffer, field_.height, field_.width, y, x, h, w, fill);
	return true;
}
...
// DSP thread operation - move a rectangular region within the field buffer
bool AhabSim::moveRect(Usz y, Usz x, Usz h, Usz w, Isz dest_y, Isz dest_x) {
	...
	gbuffer_fill_subrect(field_.buffer, field_.height, field_.width, y, x, h, w, '.');
	...
	// Clip bottom/right
	if ((Usz)dst_y + copy_h > field_.height) copy_h = field_.height - (Usz)dst_y;
	if ((Usz)dst_x + copy_w > field_.width) copy_w = field_.width - (Usz)dst_x;
	if (copy_h == 0 || copy_w == 0) {
		return true;
	}

	// Copy from tmp buffer into the field buffer at dst position
	gbuffer_copy_subrect(tmpbuf, field_.buffer, h, w,
			field_.height, field_.width, src_off_y, src_off_x, (Usz)dst_y, (Usz)dst_x, copy_h, copy_w);
	return true;
}
...
void AhabSim::setFieldSize(Usz height, Usz width) {
	...
	for (Usz ry = 0; ry < min_h; ++ry) {
		...
		Glyph *src_row = field_.buffer + ry * old_w;
	...
	tmp.width = (U16)width;
	// Copy tmp into the field buffer
	field_copy(&tmp, &field_);
	mbuf_reusable_ensure_size(&mbuf_, height, width);
	if (mbuf_.buffer) {
		mbuf_uninit_mark(mbuf_.buffer, height, width);
	}
}
...
void AhabSim::undo(Glyph* redoBuf) {
	...
	mbuf_reusable_ensure_size(&mbuf_, field_.height, field_.width);
...
void AhabSim::reset() {
	...
	// Clear the field buffer and mark buffer
	if (h > 0 && w > 0) {
		gbuffer_fill_subrect(field_.buffer, field_.height, field_.width, 0, 0, h, w, '.');
	}
	if (mbuf_.buffer) {
		mbuffer_clear(mbuf_.buffer, field_.height, field_.width);
		mbuf_uninit_mark(mbuf_.buffer, field_.height, field_.width);
	}
...
bool AhabSim::pasteCells(Glyph* cells, Usz h, Usz w, Usz dest_y, Usz dest_x) {
	...
	Usz max_h = field_.height;
	Usz max_w = field_.width;
	...
	if (copy_h == 0 || copy_w == 0) return false;
	gbuffer_copy_subrect(cells, field_.buffer, h, w,
		field_.height, field_.width, src_off_y, src_off_x, dest_y, dest_x, copy_h, copy_w);
	return true;
}
...
Usz AhabSim::getFieldHeight() const {
	...
	return (Usz)field_.height;
}

Usz AhabSim::getFieldWidth() const {
	// Lock-free read from any thread
	return (Usz)field_.width;
}
...
// DSP thread operation - step the simulation
void AhabSim::step() {
	// Ensure mbuf is large enough for this field
	mbuf_reusable_ensure_size(&mbuf_, field_.height, field_.width);
	mbuffer_clear(mbuf_.buffer, field_.height, field_.width);
	oevent_list_clear(&oevent_list_);
...
void AhabSim::process() {
	...
	// Drain UI->DSP command queue and apply each command on the DSP buffer.
