...
The fun begins when you use the port labelled _SLOT_ for selecting preset slots by CV. Although there are eight slots available it is possible to use less slots for sequencing: You can adjust the number of useable slots (i.e. sequence length) by long-pressing a slot-button while in read-mode. The LED turns off completely for slots that are currently disabled.
...
- 8FACE is using _Safe-mode_ as default setting for loading presets (added in v2.2.0). _Safe-mode_ loads presets in a slower way (technically said, preset-loading in done on the next rendered frame on the screen), but is stable. Disabling _Safe-mode_ will load presets faster, but can cause crashes or other issues. _Safe-mode_ won't make any noticable difference if snapshots are loaded manually by clicking snapshot buttons.
